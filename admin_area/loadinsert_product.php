...
if (isset($_POST['insert_product'])) {
    // Adatok fogadása és prepared statement használata
    ...
    if (empty($product_name) || empty($product_price) || empty($product_desc) || empty($product_stock) ||
        ...
        // Kép méret és típus ellenőrzése (példában feltételezem, hogy csak képfájlokat fogadunk el)
        ...
        // Kép mentése a mappába
        move_uploaded_file($temp_image, "./product_images/$product_image");

        // Feltöltés az adatbázisba prepared statementtel
        $insert_products = $con->prepare("INSERT INTO products (product_name, price, description, stock, color_ID, type_ID, material_ID, image) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $insert_products->bind_param('ssssssss', $product_name, $product_price, $product_desc, $product_stock, $product_color, $product_type, $product_material, $product_image);

        if ($insert_products->execute()) {
            echo "<script>alert('A termék feltöltése sikeresen megtörtént')</script>";
        } else {
            echo "<script>alert('A feltöltés során hiba lépett fel: " . $con->error . "')</script>";
        }
